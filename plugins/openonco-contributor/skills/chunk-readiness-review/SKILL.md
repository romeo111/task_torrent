...
The required chunk shape is enforced by the linter at `tasktorrent/lint_chunk_spec.py`. When that file changes, this skill's checks should track it. Do not duplicate the linter logic — reference it.

The linter's authoritative lists (do not paraphrase from memory; consult the file when reviewing):
...
- Defer to `tasktorrent/lint_chunk_spec.py` when the linter and your read disagree.
